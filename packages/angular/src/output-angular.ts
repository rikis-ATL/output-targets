...
import { createAngularIndividualComponents } from './create-angular-individual-components';
...
export async function angularDirectiveProxyOutput(
  ...
  if (outputTarget.generateIndividualComponents) {
    // Generate individual component files for tree shaking
    await createAngularIndividualComponents(compilerCtx, filteredComponents, pkgData, outputTarget, config.rootDir as string);
    
    // Create a proxies.ts file that re-exports from components.ts for backward compatibility
    const proxiesReExportContent = createProxiesReExportFile(filteredComponents, outputTarget);
    
    await Promise.all([
      compilerCtx.fs.writeFile(outputTarget.directivesProxyFile, proxiesReExportContent),
      copyResources(config, outputTarget),
      generateAngularDirectivesFile(compilerCtx, filteredComponents, outputTarget),
      generateValueAccessors(compilerCtx, filteredComponents, outputTarget, config),
    ]);
  } else {
    // Generate the traditional monolithic proxy file
...
function createProxiesReExportFile(components: ComponentCompilerMeta[], outputTarget: OutputTargetAngular): string {
  const autogeneratedComment = `/* tslint:disable */
...
`;
  ...
  const componentOutputPath = outputTarget.componentOutputDir || proxiesDir;
  const relativePath = path.relative(proxiesDir, componentOutputPath);
  const importPrefix = relativePath ? `${relativePath}/` : './';

  const exports = components.map((component) => {
    const tagNameAsPascal = dashToPascalCase(component.tagName);
    return `export { ${tagNameAsPascal} } from '${normalizePath(importPrefix + component.tagName)}';`;
  }).join('\n');

  return autogeneratedComment + exports + '\n';
}
