...
/**
 * Generates a post-build script that sets up tree-shaking exports in the Angular library.
 * This script automatically configures package.json subpath exports to enable individual
 * component imports for optimal bundle sizes.
 */
export async function generateTreeShakingExportScript(
  ...
  const scriptPath = path.join(scriptsDir, 'setup-tree-shaking.js');
...
export async function updatePackageJsonForTreeShaking(
  ...
  try {
    ...
    if (!packageJson.scripts['build:prod']) {
      packageJson.scripts['build:prod'] = 'ng build --configuration production && node scripts/setup-tree-shaking.js';
    } else if (!packageJson.scripts['build:prod'].includes('setup-tree-shaking')) {
      // Add to existing build:prod script
      packageJson.scripts['build:prod'] += ' && node scripts/setup-tree-shaking.js';
    }
